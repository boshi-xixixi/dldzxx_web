...
export function generateEmployeeReport(employeeId: string) {
  ...
  return {
    employee,
    summary: {
      totalOnlineTime: Math.floor(Math.random() * 8) + 1,
      totalTraffic: Math.floor(Math.random() * 1000) + 100,
      websitesVisited: Math.floor(Math.random() * 50) + 10,
...
export function generateAIUsageReport() {
  return {
      ...
      chapters: [
        {
          ...
          stats: { totalQueries: 45, departments: ['技术部', '市场部', '人事部'], topQuestions: ['代码优化', '数据分析', '文档生成'] },
          mood: 'productive',
        },
        {
          title: '午间协作',
          ...
          stats: { totalQueries: 28, departments: ['全部门'], topQuestions: ['健康咨询', '学习规划', '工作总结'] },
          mood: 'relaxed',
        },
        {
          title: '下午冲刺',
          ...
          stats: { totalQueries: 67, departments: ['财务部', '运营部', '技术部'], topQuestions: ['数据处理', '流程优化', '问题解决'] },
          mood: 'intense',
        },
      ],
      insights: [
    ...
    visualData: {
      usageTimeline: Array.from({ length: 24 }, (_, i) => ({ hour: i, queries: Math.floor(Math.random() * 20), users: Math.floor(Math.random() * 10) })),
      departmentUsage: { 技术部: 45, 市场部: 32, 财务部: 28, 人事部: 20, 运营部: 15 },
      questionTypes: [
